...
function inject (bot) {
  ...
  // 26.1.2 (1.20.5+) 新版速度包 - 击退用喵！
  bot._client.on('set_entity_motion', (packet) => {
    const entity = fetchEntity(packet.entityId)
    if (!entity) return
    // 新版 velocity 是 double 类型，已经是 blocks/tick，不需要转换喵
    // 兼容两种字段命名：velocity.x/y/z 或 velocityX/Y/Z
    const vx = packet.velocityX ?? packet.velocity?.x ?? packet.x ?? 0
    const vy = packet.velocityY ?? packet.velocity?.y ?? packet.y ?? 0
    const vz = packet.velocityZ ?? packet.velocity?.z ?? packet.z ?? 0
